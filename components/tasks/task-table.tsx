            ...
            <table className="w-full min-w-[780px] text-left text-sm">
              ...
              <tbody className="divide-y divide-white/10">
                ...
                {!isProcessing && items.map((task) => (
                  <tr key={task.id} className="bg-white/[0.018] transition hover:bg-white/[0.03]">
                ...
                {!isProcessing && items.length === 0 && (
                  <tr className="bg-white/[0.018]">
                    <td colSpan={6} className="px-4 py-10 text-center text-sm text-muted-foreground">
                      <div className="mx-auto max-w-sm rounded-lg border border-dashed border-white/15 bg-white/[0.03] px-4 py-6">
                        <p className="font-medium text-white">No MAP tasks match the current view</p>
                        <p className="mt-2 text-sm text-muted-foreground">
                          Clear search or relax filters to bring the banking compliance tasks back into view.
                        </p>
                      </div>
                    </td>
                  </tr>
